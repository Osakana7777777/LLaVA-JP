...
if __name__=="__main__":
    ...
    cc_300k_dataset.to_csv('dataset/cc_300k_dataset.csv')
    cc_300k_df = pd.read_csv('dataset/cc_300k_dataset.csv')
    img_dataset = load_dataset("common-canvas/commoncatalog-cc-by", split="train", streaming=True)
    data_info = []


    for i, data in tqdm(enumerate(img_dataset), total=300_000):
        data["jpg"].save(f"dataset/images/cc_300k/{i:09}.jpg")
    ...
    chat_ja_path = Path('dataset', 'LLaVA-CC-300k-Caption', 'llava_cc_300k_caption.json')
    with open(chat_ja_path, mode="w") as f:
        json.dump(cc_300k_llava_formats, f, indent=2, ensure_ascii=False)



# _ = Parallel(n_jobs=-1)(delayed(load_images)(i, data) for i, data in tqdm(enumerate(img_dataset), total=300_000))
# _ = Parallel(n_jobs=-1)(delayed(process_data)(i) for i in tqdm(enumerate(img_dataset), total=300_000))
